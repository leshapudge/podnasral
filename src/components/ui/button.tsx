...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-primary-foreground mc-border hover:brightness-110 active:translate-y-0.5",
        gold:
          "bg-hypixel-gold text-accent-foreground mc-border-gold hover:brightness-110 active:translate-y-0.5",
        secondary:
...
export interface ButtonProps
  ...
  asChild?: boolean;
  /** Disable UI hover/click sounds */
  silent?: boolean;
}

const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, silent = false, disabled, ...props }, ref) => {
