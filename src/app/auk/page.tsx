  ...
  return (
    <McPageShell title="Аук-донаты" closeHref="/">
      <section className="rounded border border-[#1a1208] bg-[#1a1208]/60 p-6">
        {feed.event ? (
          <div className="text-sm text-[#d6c3a1]">
            Событие: <span className="font-semibold text-[#e8d5b0]">{feed.event.name}</span>{" "}
            ({feed.event.status})
          </div>
        ) : (
      ...
      <section className="rounded border border-[#1a1208] bg-[#1a1208]/60 p-6">
        ...
        <p className="text-sm text-[#d6c3a1]">
          Выбор игры и запуск забега перенесены в панель стримера. Здесь остается только лента
          донатов.
        </p>
      </section>
